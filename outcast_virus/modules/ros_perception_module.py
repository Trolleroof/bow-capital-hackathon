...
Subscribes to YOLOX ROS2 topics and publishes normalized Outcast Virus bus topics.
...
class RosPerceptionModule(AbstractModule):
    ...
    async def run(self) -> None:
        if not config.ENABLE_ROS_PERCEPTION:
            log.info("[ros_perception] disabled; set OUTCAST_VIRUS_ROS_PERCEPTION=1 to enable")
            while True:
        ...
        self._stop.clear()
        self._thread = threading.Thread(target=self._spin_ros, name="outcast_virus_ros_perception", daemon=True)
        self._thread.start()
    ...
    def _spin_ros(self) -> None:
        ...
        node = Node("outcast_virus_orchestrator_perception_bridge")
        bridge = CvBridge()
