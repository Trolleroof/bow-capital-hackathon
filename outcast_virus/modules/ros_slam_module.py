...
This module runs inside the Outcast Virus orchestrator on the desktop. It subscribes
to ROS2 topics over DDS from the Jetson and publishes normalized Outcast Virus bus
...
class RosSlamModule(AbstractModule):
    ...
    async def run(self) -> None:
        if not config.ENABLE_ROS_SLAM:
            log.info("[ros_slam] disabled; set OUTCAST_VIRUS_ROS_SLAM=1 to subscribe to ROS2 topics")
            while True:
        ...
        self._stop.clear()
        self._thread = threading.Thread(target=self._spin_ros, name="outcast_virus_ros_slam", daemon=True)
        self._thread.start()
    ...
    def _spin_ros(self) -> None:
        try:
            ...
            log.error("[ros_slam] source ROS2 and install cv_bridge/python3-opencv before enabling OUTCAST_VIRUS_ROS_SLAM")
            return

        if not rclpy.ok():
            rclpy.init(args=None)

        node = Node("outcast_virus_orchestrator_slam_bridge")
