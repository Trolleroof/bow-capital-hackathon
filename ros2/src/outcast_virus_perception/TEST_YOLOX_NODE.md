...
This guide tests `outcast_virus_perception/yolox_node.py` by itself on the Jetson, before involving the orchestrator or frontend.

## Prerequisites

- ROS2 Humble is installed and sourced.
- The OAK-D camera can publish an image topic such as `/oak/rgb/image_rect`.
- The `outcast_virus_perception` package has been built in the ROS2 workspace.
...
colcon build --symlink-install --packages-select outcast_virus_perception
...
```bash
ros2 run outcast_virus_perception yolox_node --help
```
...
ros2 run outcast_virus_perception yolox_node --ros-args \
