...
{   
    ...
    private:
        ...
        std::string packageShareDir = ""; // Package share directory (dynamically discovered)
        ...
        // Direct camera subscription parameters
        std::string directImageTopic = ""; // Topic to subscribe directly to camera (e.g., /oak/rgb/image_rect)
        std::string directSettingsName = ""; // Settings name passed directly via parameter
        bool useDirectSubscription = false; // Flag to use direct camera subscription mode
        ...
        rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr directImgMsg_subscription_; // Direct camera subscription
        ...
        void DirectImg_callback(const sensor_msgs::msg::Image& msg); // Callback for direct camera subscription (extracts timestamp from header)
