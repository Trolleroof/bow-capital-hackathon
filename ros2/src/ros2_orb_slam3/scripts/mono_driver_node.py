...
class MonoDriver(Node):
    def __init__(self, node_name = "mono_py_node"):
        ...
        try:
            package_share_dir = ament_index_python.packages.get_package_share_directory('ros2_orb_slam3')
        except Exception as e:
            self.get_logger().error(f"Failed to find package share directory: {e}")
            raise

        self.package_dir = package_share_dir
        ...
        self.image_sequence_dir = self.package_dir + "/" + self.parent_dir + "/" + self.image_seq # Full path to the image sequence folder
